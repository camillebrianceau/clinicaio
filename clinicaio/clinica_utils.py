...
def container_from_filename(bids_or_caps_filename: Path) -> Path:
    ...
    m = re.search(r"(sub-[a-zA-Z0-9]+)/(ses-[a-zA-Z0-9]+)", bids_or_caps_filename) # type: ignore
    if not m:
        raise ValueError(
...
def _common_checks(directory: Path, folder_type: str) -> None:
    ...
    error = ClinicaIOBIDSError if folder_type == "BIDS" else ClinicaIOCAPSError
...
def check_caps_folder(caps_directory: Path) -> None:
    ...
    if len(sub_folders) > 0:
        ...
        raise ClinicaIOCAPSError(error_string)
...
def find_sub_ses_pattern_path(
    ...
    if len(current_glob_found) > 1:
        ...
        if _are_multiple_runs(current_glob_found): # type: ignore
...
def clinicadl_file_reader(
    ...
    ClinicaIOCAPSError or ClinicaIOBIDSError
    ...
    * More than 1 file found::
    ...
    if len(errors_encountered) > 0 and raise_exception:
        if is_bids:
            raise ClinicaIOBIDSError(error_message)
        else:
            raise ClinicaIOCAPSError(error_message)
...
def fetch_file(remote: RemoteFileStructure, dirname: Path) -> Path:
    ...
    try:
        ...
        if hasattr(e, "reason"):
            ...
            cprint(
                msg=f"The server could not fulfill the request. Error code: {e.code}", # type: ignore
                lvl="error",
            )
    else:
        try:
